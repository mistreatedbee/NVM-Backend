...
exports.createDispute = async (req, res, next) => {
  try {
    if (!['customer', 'vendor'].includes(req.user.role)) {
      return res.status(403).json({ success: false, message: 'Only customers or vendors can open disputes' });
    }

    const { orderId, reason, description, reasonCategory } = req.body || {};
    ...
    const baseOrderQuery = { _id: orderId };
    if (req.user.role === 'customer') {
      baseOrderQuery.$or = [{ customer: req.user.id }, { customerId: req.user.id }];
    }
    const order = await Order.findOne(baseOrderQuery).select('_id orderNumber items customer customerId');
    if (!order) return res.status(404).json({ success: false, message: 'Order not found' });

    let vendorId = String(req.body.vendorId || '') && isObjectId(req.body.vendorId)
      ? req.body.vendorId
    ...
    if (req.user.role === 'vendor') {
      ...
      const inOrder = (order.items || []).some((item) => String(item.vendorId || item.vendor) === String(vendorProfile._id));
      if (!inOrder) return res.status(403).json({ success: false, message: 'Order is not linked to your store' });
    ...
    const dispute = await Dispute.create({
      order: order._id,
      orderId: order._id,
      customer: req.user.role === 'customer' ? req.user.id : (order.customer || order.customerId),
      vendor: vendorId,
      reason: reasonText,
      reasonCategory: String(reasonCategory || 'OTHER').toUpperCase(),
      description: descriptionText,
      evidence: attachments,
      evidenceUrls: attachments.map((item) => item.url).filter(Boolean),
      openedByUserId: req.user.id,
      openedByRole: req.user.role === 'vendor' ? 'VENDOR' : 'CUSTOMER',
      status: 'OPEN',
      messages: [
        {
          sender: req.user.id,
          senderRole: req.user.role === 'vendor' ? 'VENDOR' : 'CUSTOMER',
